...
3. 识别关键的类和函数，各一句话概括
4. 用 `record_context_point` 创建锚点节点：
   - title: 模块名 + 一句话职责
...
4. 用 `record_point` 记录一条因果关系，再用 `record_line` 连线到相关节点：
   - title: "A → B: 一句话描述协作关系"
...
class ActionHistory:
    ...
    def record_round(self, round_num: int, round_events: list):
        for event in round_events:
            ...
            if not name or name in ("search_knowledge_nodes", "record_lesson_node", "record_point", "record_line"):
                continue
        ...
        try:
            process_coro = agent.process(
                ...
                loop_config={
                    ...
                    "gp_unblock_tools": ["record_point", "record_line", "record_context_point"],
