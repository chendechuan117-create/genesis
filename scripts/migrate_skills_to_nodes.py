#!/usr/bin/env python3
"""
技能迁移脚本 - 将 skills/ 目录下的 Python 工具文件迁移到 NodeVault 中
...
def migrate_skill_file(file_path: Path, vault: NodeVault) -> bool:
    ...
    try:
        ...
        # 检查是否已存在
        existing = vault.get_node(node_id)
        if existing:
...
def main():
    ...
    # 验证迁移结果
    logger.info("\n验证迁移结果:")
    tool_nodes = vault.search_nodes(keywords=["TOOL"], ntype="TOOL")
    logger.info(f"数据库中的 TOOL 节点数量: {len(tool_nodes)}")
    
    # 显示部分迁移的工具
    if tool_nodes:
        logger.info("\n已迁移的工具节点示例:")
        for i, node in enumerate(tool_nodes[:5]):
