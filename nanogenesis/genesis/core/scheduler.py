    ...
    def _load_jobs(self):
        ...
        try:
            ...
            with self.db_path.open('r', encoding='utf-8') as f:
                ...
                # Merge logic: if job exists, keep runtime state (next_run), update config
                # Actually simpler to just replace non-runtime properties?
                # Or just replace entire dict but try to carry over next_run?
                
                # Simplest for now: Replace self.jobs, but try to preserve next_run for existing IDs
                for jid, new_job in new_jobs.items():
                    if jid in self.jobs:
                        new_job.last_run = self.jobs[jid].last_run
                        new_job.next_run = max(new_job.next_run, self.jobs[jid].next_run) # Respect the later run time
                        
                self.jobs = new_jobs
                
            logger.info(f"✓ (Re)Loaded {len(self.jobs)} background tasks")
