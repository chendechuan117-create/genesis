...
class NetworkOptimizationAdvisor:
    def __init__(self):
        self.name = "network_optimization_advisor"
    ...
    def execute(self, diagnostic_data: str) -> Dict:
        """分析诊断数据并生成优化建议"""
    ...
    def _parse_diagnostics(self, data: str) -> Dict:
        """解析诊断数据"""
        metrics = {
            "mtu": 1500,  # 默认值
            "rx_errors": 0,
    ...
    def _generate_recommendations(self, metrics: Dict) -> List[Dict]:
        ...
        # 5. 网卡特性优化
        if "tcp-segmentation-offload: off" in str(metrics.get("nic_features", "")):
            recommendations.append({
                "id": "tso_optimization",
                "title": "启用TSO（TCP分段卸载）",
                "description": "TSO可降低CPU负载，提升网络吞吐量",
                "priority": "medium",
                "risk": "medium",
                "impact": "high"
            })
        ...
        if metrics["tcp_connections"] > 1000:
            recommendations.append({
                ...
                "title": "优化TCP连接管理",
                "description": f"当前TCP连接数{metrics['tcp_connections']}较高，可调整连接限制",
                "priority": "low",
                "risk": "medium",
                "impact": "medium"
            })
    ...
    def _generate_commands(self, recommendations: List[Dict]) -> Dict[str, List[str]]:
        """生成可执行的优化命令"""
        commands = {}
        ...
        for rec in recommendations:
            ...
            elif rec["id"] == "connection_optimization":
                commands[rec["id"]] = [
                    "# 优化连接限制",
