...
class V4Loop(LensPhaseMixin, CPhaseMixin):
    ...
    def __init__(
        ...
        self._token_history: List[int] = []  # instance-local token window

        # 启动时恢复 persona 学习数据（只在首次 V4Loop 实例化时加载一次）
        Blackboard.load_from_db()

    # Token efficiency: instance-level sliding window
    @property
    def _TOKEN_WINDOW(self) -> int:
        return PIPELINE_CONFIG.token_window_size

    @property
    def _token_window(self) -> int:
        return self._TOKEN_WINDOW

    def _record_token_usage(self, total_tokens: int):
        self._token_history.append(total_tokens)
        if len(self._token_history) > self._token_window:
            self._token_history = self._token_history[-self._token_window:]

    def get_token_efficiency_stats(self) -> Optional[Dict[str, Any]]:
        if not self._token_history:
            return None
        avg = sum(self._token_history) / len(self._token_history)
        return {
            "window_size": len(self._token_history),
            "avg_tokens_per_request": round(avg),
            "last_tokens": self._token_history[-1] if self._token_history else 0,
            "max_tokens": max(self._token_history),
            "min_tokens": min(self._token_history),
        }

    async def run(self, user_input: str, step_callback: Any = None, image_paths: Optional[List[str]] = None, loop_config: Optional[Dict[str, Any]] = None, initial_knowledge_state: Optional[Dict[str, Any]] = None, knowledge_cursor: Optional[Dict[str, Any]] = None, session_id: Optional[str] = None) -> Tuple[str, PerformanceMetrics]:
        ...
        self.potential_baseline: Dict[str, int] = {}  # per-call baseline
        ...
        if c_mode == "FULL" and self.metrics.total_tokens > 0:
            ...
            if self.metrics.total_tokens > TOKEN_BUDGET_THRESHOLD:
                logger.warning(
                    ...
                    f"(>{TOKEN_BUDGET_THRESHOLD}), downgrading Maintenance FULL→LIGHT"
                )
                c_mode = "LIGHT"
        if c_mode != "SKIP":
            if self.c_phase_blocking:
                await self._run_c_phase_safe(step_callback, c_mode, g_final_response=final_response)
                logger.info(f"Maintenance completed (blocking mode={c_mode}, GP={self.metrics.g_tokens}t).")
            else:
                task = asyncio.create_task(self._run_c_phase_safe(step_callback, c_mode, g_final_response=final_response))
                task.add_done_callback(lambda t: t.exception() and logger.error(f"Maintenance background task failed: {t.exception()}") if not t.cancelled() else None)
                logger.info(f"Maintenance launched in background (mode={c_mode}, GP={self.metrics.g_tokens}t).")
        else:
            logger.info(f"Skipping Maintenance: mode=SKIP (GP={self.metrics.g_tokens}t).")
        ...
        for i in range(gp_max_iterations):
            ...
            if response.tool_calls:
                ...
                for tc, should_exec, skip_reason, is_safe in exec_plan:
                    ...
                    # Report delta using per-call baseline (not round-level counter)
                    if tc.name == "search_knowledge_nodes" and hasattr(self.vault, "count_potential_samples"):
