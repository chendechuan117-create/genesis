...
AUTO_DEFAULT_DIRECTIVE = (
    "围绕 Genesis 的 PLS 核心进行自主概念探索。Genesis 的核心不是工具系统本身，而是 PLS："
    "Point/Line/Surface 如何把经验压缩成可复用概念地形，并在后续轮次塑造 LLM 的注意力、判断和行动。"
    "每轮选择一个影响 PLS 生产、连接、激活、消费、反馈或演化的概念缺口。"
    "可以观察代码、工具、trace、Doctor、runtime 或 skill，但它们只是证据；如果一个外围现象不能说明 PLS 的机制、边界、失败模式或改进方向，就不要继续深挖。"
    "目标不是泛化研究 Genesis，也不是工具考古，而是让 PLS 这个核心系统变得更清楚、更可复用、更能闭环。"
    "每轮只做一件事，做到位。"
)
