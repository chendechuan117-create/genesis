...
os.chdir('/home/chendechusn/Genesis/Genesis')

# 加载 .env
from dotenv import load_dotenv
load_dotenv('/home/chendechusn/Genesis/Genesis/.env')
...
def test_config():
    ...
    def t_discord_stripped():
        ...
        has_discord = 'DISCORD_TOKEN' in content.upper() or 'DISCORD_BOT' in content.upper()
        assert not has_discord, "DISCORD_TOKEN found in sandbox .env!"
...
def test_provider():
    ...
    def t_api_live():
        import asyncio
        from genesis.core.provider import NativeHTTPProvider
        key = os.getenv('AIXJ_API_KEY')
        if not key:
            return "SKIP"
        p = NativeHTTPProvider(
            api_key=key, base_url='https://aixj.vip/v1',
            default_model='gpt-5.4', provider_name='aixj',
            skip_content_type=True, request_timeout=30, wall_clock_timeout=60
        )
        r = asyncio.run(p.chat([{'role':'user','content':'Reply OK'}], max_tokens=5))
        assert r.content, "Empty response"
        return f"gpt-5.4 replied: '{r.content[:50]}'"
    test("LLM API live test (aixj/gpt-5.4)", t_api_live)
...
def test_nodevault():
    ...
    def t_node_count():
        ...
        from pathlib import Path
        vault = NodeVault(db_path=Path('/home/chendechusn/Genesis/Genesis/runtime/genesis_v4.db'), skip_vector_engine=True)
        cur = vault._conn.execute("SELECT ntype, COUNT(*) FROM knowledge_nodes GROUP BY ntype")
