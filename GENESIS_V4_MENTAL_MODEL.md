...
### 3.3 V4Loop 内部状态机 (Subroutine 模式)
...
```
[Phase 1: G-Process (主循环)]
  ├── 系统注入：历史记忆 + 检索工具权限
  ├── 循环调用 LLM
  │    ├── 如果调用 `search_knowledge_nodes`，执行并继续。
  │    ├── 如果输出纯文本包含 ````dispatch` 代码块，则解析提取 Payload。
  │    │     └── 挂起 G 进程，进入 Phase 2 (呼叫子程序)
  │    └── 如果输出普通纯文本（且无 payload），视为对用户的最终回复，结束任务。
  └── 异常兜底：若超时未输出有效回复，则报告构建失败。

[UI Rendering]
  ├── 将 Payload 解析并渲染为可视化组件发送给用户 ("🧠 大脑已完成思考，正在派发任务...")

[Phase 2: Op-Process (子程序)]
...
[Phase 1 Resumed: G-Process (总结)]
  ├── G 收到系统注入消息：`[Op-Process 执行完毕] 返回结果如下...`
  └── G 结合最初的用户意图和 Op 的执行结果，生成带有人情味的最终回答。

[Phase 3: C-Process (Post-Execution)]
  ├── 将 Op-Process 的纯净执行轨迹压缩为摘要。
  ├── 唤醒 C 进程，注入摘要和写入权限工具 (`record_*`)。
  └── C 进程分析并调用工具保存知识，或回复 NO_ACTION。
```

**关键细节**：
- **Subroutine Model (A -> b -> A)**: G 并没有在派发任务后下班，而是等待 Op 执行完毕，并根据 Op 的结果进行“回声”包装，提供连贯的最终答复。
- **Context Firewalling**: Op 依然是个瞎子，它看不到用户的闲聊，只能看到 G 的指令。这确保了它不受情绪干扰。
- **Output Truncation**: Shell 输出截断 (4000 chars), File 读取截断 (8000 chars)。防止 Context 爆炸。
- **Reflection Summary**: C 进程只反思 Op 的执行轨迹，不看 G 的聊天记录，确保提纯的都是“硬核经验”。
...
*本文档生成时间: 2026-03-12, 基于 Genesis V4.2 (Glassbox) 全部源码审阅。最后更新: 2026-03-15*

---

## 十二、 生态进化：Genesis Experience Graph (V4.3)

受 **GitNexus** 的 "Code Graph" 启发，Genesis V4.3 将从扁平的文档检索升级为 **图谱增强检索 (Graph RAG)**。

我们不再存储孤立的“经验文档”，而是存储由 **实体 (Nodes)** 和 **关系 (Edges)** 构成的动态图谱。

### 12.1 节点类型 (Nodes)

将原有的 `CONTEXT` 和 `LESSON` 拆解为更原子的单元：

| 类型 | ID 前缀 | 含义 | 示例 |
|---|---|---|---|
| **ENTITY** | `ENT_*` | 静态对象/工具/服务 | `ENT_PYTHON`, `ENT_GITHUB`, `ENT_PROXY_CONFIG` |
| **EVENT** | `EVT_*` | 发生的现象 (报错/结果) | `EVT_SSL_ERROR`, `EVT_TIMEOUT`, `EVT_INSTALL_SUCCESS` |
| **ACTION** | `ACT_*` | 具体的执行动作/命令 | `ACT_PIP_INSTALL`, `ACT_SET_PROXY`, `ACT_CHECK_PORT` |
...
| 关系 | 方向 | 含义 | 示例 |
|---|---|---|---|
| **TRIGGERS** | Action -> Event | 动作触发了现象 | `ACT_PIP_INSTALL` -> `EVT_SSL_ERROR` |
| **RESOLVES** | Action -> Event | 动作解决了现象 | `ACT_USE_MIRROR` -> `EVT_SSL_ERROR` |
| **REQUIRES** | Action -> Entity | 动作依赖某实体 | `ACT_GIT_CLONE` -> `ENT_PROXY_CONFIG` |
| **LOCATED_AT** | Entity -> Entity | 实体的物理位置 | `ENT_PROXY_CONFIG` -> `ENT_LOCALHOST_20170` |
| **RELATED_TO** | Any <-> Any | 弱相关性 | `ENT_PYTHON` <-> `ENT_PIP` |

### 12.3 运作机制 (The Graph Loop)

#### (1) 摄入 (C-Process: The Cartographer)
C 进程不再写日记，而是**画图**。
*   **输入**: Op 的执行日志 (Log)。
*   **Prompt**: "从日志中提取实体和因果关系。"
*   **输出**:
    ```json
    [
      {"source": "ACT_CURL_GITHUB", "relation": "TRIGGERS", "target": "EVT_TIMEOUT"},
      {"source": "ACT_SET_PROXY", "relation": "RESOLVES", "target": "EVT_TIMEOUT"}
    ]
    ```
*   **存储**: 写入 SQLite `node_edges` 表。

#### (2) 检索 (G-Process: Graph RAG)
G 进程不再只看关键词匹配，而是**顺藤摸瓜**。
*   **Query**: "pip install fail"
*   **Vector Hit**: 命中 `EVT_SSL_ERROR`。
*   **Graph Walk**: 查 `RESOLVES` 边 -> 找到 `ACT_USE_MIRROR`。
*   **Result**: 即使 `ACT_USE_MIRROR` 的文本里没有 "fail" 这个词，它也会因为**图连接**被一并带出。

### 12.4 价值
这解决了“元信息”的根本问题：
*   **结构化**: 机器可读，不再是自然语言的“散文”。
*   **完备性**: 通过 `REQUIRES` 边，G 只要想用某个工具，就必须带上它依赖的 Config。
*   **可解释性**: 每一条建议都有明确的因果链路 (`Because A triggers B, and C resolves B`).
