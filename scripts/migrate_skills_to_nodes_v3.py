#!/usr/bin/env python3
"""
技能迁移脚本 v3 - 将 skills/ 目录下的 Python 工具文件迁移到 NodeVault 中
...
def extract_tool_info_from_source(source_code: str, filename: str) -> Dict[str, Any]:
    """
    从 Python 源码中提取工具信息
    ...
    # 尝试提取类名
    class_match = re.search(r'class\s+(\w+)\s*\(.*Tool.*\):', source_code)
    if class_match:
        class_name = class_match.group(1)
        # 如果类名以 Tool 结尾，去掉 Tool 后缀
    ...
    # 提取 description
    desc_match = re.search(r'@property\s*\ndef\s+description\s*\(self\)\s*->\s*str:\s*return\s+"([^"]+)"', source_code, re.DOTALL)
    if not desc_match:
        desc_match = re.search(r'@property\s*\ndef\s+description\s*\(self\)\s*->\s*str:\s*return\s+\'([^\']+)\'', source_code, re.DOTALL)
    if not desc_match:
        # 尝试更宽松的匹配
...
def migrate_skill_file(file_path: Path, vault: NodeVault) -> bool:
    ...
    try:
        ...
        # 提取工具信息
        tool_info = extract_tool_info_from_source(source_code, file_path.name)
...
def main():
    ...
    if tool_nodes:
        logger.info("\n已迁移的工具节点示例:")
        for i, node in enumerate(tool_nodes[:15]):
