...
class NodeVault(EnvironmentEpochMixin, ArenaConfidenceMixin):
    ...
    def add_edge(self, source_id: str, target_id: str, relation: str, weight: float = 1.0):
        """添加一条图谱边 (Idempotent, VOID-safe)"""
        # Prevent VOID edges: validate both endpoints exist before INSERT
        try:
            cur = self._conn.execute(
                "SELECT 1 FROM knowledge_nodes WHERE node_id = ? UNION SELECT 1 WHERE node_id = ?",
                (source_id, target_id)
            )
            endpoints_found = len(cur.fetchall())
            if endpoints_found < 2:
                logger.warning(f"Graph: Refused VOID edge {source_id} --[{relation}]--> {target_id} "
                               f"(source_ok={endpoints_found > 0}, target_ok={endpoints_found > 1})")
                return
            self._conn.execute(
    ...
    def record_potential_samples(self, samples: List[Dict[str, Any]], trace_id: str = None, round_seq: int = None, source: str = "surface") -> int:
        try:
            ...
            self._conn.executemany(
                "INSERT INTO potential_samples (trace_id, round_seq, source, potential_type, title, detail, node_ids, evidence) VALUES (?,?,?,?,?,?,?,?)",
                rows
            )
            self._conn.commit()
            return len(rows)
