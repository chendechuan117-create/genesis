...
class TaskFlowExecutor:
    ...
    def _parse_task_steps(self, task_plan: str) -> List[Dict[str, Any]]:
        """解析任务步骤"""
        steps = []
    ...
    def _execute_step(self, step: Dict[str, Any], step_num: int, total_steps: int) -> Dict[str, Any]:
        ...
        try:
            ...
            elif "分析" in action or "analyze" in action_lower:
                result = self._execute_analyze(action, details)
            elif "执行" in action or "run" in action_lower:
                result = self._execute_command(action, details)
            elif "检查" in action or "check" in action_lower:
