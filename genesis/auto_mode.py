...
5. **记录知识时先 record_point 再 record_line**——每条线回答一个不同的因果问题：为什么此新点基于那个特定节点？不同 basis 的 reasoning 必须不同（如：CONTEXT→环境条件，PATTERN→已知模式，LESSON→先验经验）。没有推理链的知识无法判断价值、无法去重，是噪音。
...
- 每轮聚焦一个假设，做到位
- **记录知识必须连线**：先 record_point 写点，再 record_line 连线；每条线带独立 reasoning，回答为什么基于该节点（不同 basis 不同角度），没有线的知识是噪音
...
5. **如果发现非平凡的执行经验**（如隐含依赖、初始化顺序、竞态条件），用 `record_point` 记录新点，再用 `record_line` 连线到相关锚点节点，每条线的 reasoning 回答不同的因果问题
...
- 边连接由系统自动完成，你只需创建锚点
- **新点必须带线**：record_point 后必须继续调用 record_line，每条线必须带独立的 reasoning
