...
5. **记录 LESSON 时必须填写 reasoning_basis**——每条线回答一个不同的因果问题：为什么此经验基于那个特定节点？不同basis的reasoning必须不同（如：CONTEXT→环境条件，PATTERN→已知模式，LESSON→先验经验）。没有推理链的知识无法判断价值、无法去重，是噪音。
...
- 每轮聚焦一个假设，做到位
- **记录 LESSON 必须连线**：reasoning_basis 每条线带独立reasoning，回答为什么基于该节点（不同basis不同角度），没有线的知识是噪音
...
5. **如果发现非平凡的执行经验**（如隐含依赖、初始化顺序、竞态条件），用 `record_lesson_node` 记录，**必须填写 reasoning_basis** 连线到相关锚点节点，每条线的reasoning回答不同的因果问题
...
- 边连接由系统自动完成，你只需创建锚点
- **LESSON 必须带线**：reasoning_basis 为空会被拒绝，每条线必须带独立的reasoning
...
4. 用 `record_lesson_node` 记录一条推理链（**reasoning_basis 必填**，连线到两个模块的锚点节点）：
   ...
   - reasoning_basis: [{"basis_node_id": 模块A锚点ID, "reasoning": "A的什么行为依赖B"}, {"basis_node_id": 模块B锚点ID, "reasoning": "B提供了什么给A使用"}]
...
- 不要做环境检查、不要搜索知识库
- **reasoning_basis 为空会被拒绝，每条线必须带独立的reasoning**——连线是知识的骨架，没有线的知识是噪音
